  ...
  if (signupForm) {
    signupForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (user) {
          // Create profile row + start the 7-day trial.
          await DB.upsertProfile({ id: user.id, full_name: fullName, email });
          try { await DB.createTrialSubscription(user.id); } catch (e) { console.error(e); }
        }
        ...
        if (data.session) {
          ...
          showFormMsg("formMsg", "Account created! Please check your email to confirm your address, then log in.", "success");
        }
      } catch (err) {
